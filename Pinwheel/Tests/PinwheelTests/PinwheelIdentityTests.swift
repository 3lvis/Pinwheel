...
final class PinwheelIdentityTests: XCTestCase {
    func testGeneratedIDIsStableForEmojiOnlyTitle() {
        ...
        XCTAssertEqual(
            first,
            second,
            "a title that slugifies to empty must still produce a deterministic id"
        )
    ...
    func testGeneratedIDIsStableForPunctuationOnlyTitle() {
        XCTAssertEqual(PinwheelItem.generatedID(title: "•••"), PinwheelItem.generatedID(title: "•••"))
    }
    ...
    func testItemIDIsStableAcrossReadsForEmojiTitle() {
        ...
        XCTAssertEqual(
            item.id,
            item.id,
            "a computed id must not change between reads"
        )
    }
