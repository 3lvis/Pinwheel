...
final class PinTrayGeometryTests: XCTestCase {
    ...
    func testATrayArrivesFromBelowItsOwnBottomEdge() {
        ...
        XCTAssertGreaterThan(
            ...
            0,
            "a tray travels up to its place rather than appearing there"
        )
    }
    ...
    func testAPullUpResistsAndIsBounded() {
        XCTAssertEqual(
            PinTrayGeometry.travel(forDrag: 200),
            200,
            "a tray on its way out follows the finger"
        )
        ...
        XCTAssertLessThan(
            gentle,
            0,
            "a pull up lifts the tray"
        )
        XCTAssertGreaterThan(
            gentle,
            -40,
            "but by less than the finger came"
        )

        XCTAssertLessThan(
            PinTrayGeometry.travel(forDrag: -400),
            gentle,
            "pulling harder lifts it further"
        )
        XCTAssertGreaterThan(
    ...
    func testATrayThatIsNotEditingRestsAtTheBottomWhileTheKeyboardLeavesOverIt() {
        ...
        XCTAssertEqual(
            resting.bottomInset,
            trayBottomMargin,
            "only the keyboard moves on the way out"
        )
    ...
    func testContentKeepsAFloorOfClearanceWhereThereIsNoHomeIndicator() {
        let noIndicator = PinTrayGeometry(contentHeight: 300, room: PinTrayGeometry.Room(containerHeight: 912, safeAreaTop: 20, safeAreaBottom: 0))
...
extension PinTrayGeometryTests {
    func testTheBackdropArrivesAndLeavesWithTheTray() {
        ...
        let leaving = PinTrayGeometry(
            contentHeight: 400,
            room: room,
            phase: .leaving
        )
        let arriving = PinTrayGeometry(
            contentHeight: 400,
            room: room,
            phase: .arriving
        )
        ...
        XCTAssertEqual(
            standing.dimming,
            1,
            "a tray on screen is read against a dimmed backdrop"
        )
        XCTAssertEqual(
            arriving.dimming,
            0,
            "one on its way in brings the backdrop up with it"
        )
        XCTAssertEqual(
            leaving.dimming,
            0,
            "a tray on its way out takes the backdrop with it"
        )
    }
    ...
    func testAReleaseWithNoSpeedStartsTheSpringAtRest() {
        XCTAssertEqual(
            PinTrayGeometry.springVelocity(travelling: 200, releasedAt: 0),
            0,
            accuracy: 0.001,
    ...
    func testAFlickThatHasNowhereLeftToTravelDoesNotLaunchTheSpring() {
        XCTAssertEqual(
            PinTrayGeometry.springVelocity(travelling: 0, releasedAt: 1200),
            0,
            accuracy: 0.001,
...
extension PinTrayGeometryTests {
    func testTheBackdropDimsInStepWithHowFarTheTrayStillHasToGo() {
        ...
        XCTAssertEqual(
            ...
            1,
            accuracy: 0.001,
            "a tray standing still dims what it covers fully"
        )
    ...
    func testTheBackdropIsClearOnceTheTrayHasGone() {
        XCTAssertEqual(
            geometry(phase: .leaving).dimming,
            0,
            accuracy: 0.001,
            "nothing left to dim behind"
        )
        XCTAssertEqual(
            geometry(phase: .arriving).dimming,
            0,
            accuracy: 0.001,
