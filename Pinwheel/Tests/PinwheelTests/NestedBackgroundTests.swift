...
final class NestedBackgroundTests: XCTestCase {
    ...
    func testNestedContainerDropsItsParentsDuplicatedBackground() {
        ...
        let content = FigmaNode(
            ...
            h: 80,
            fill: fill,
            fillToken: "secondaryBackground",
            radius: 12,
            radiusToken: "radius-m",
            layout: columnLayout(pad: 12),
            children: [text]
        )
        let thumbnail = FigmaNode(
            tag: "frame",
            x: 0,
            y: 0,
            w: 64,
        ...
        XCTAssertEqual(
            inner.layout?.pad,
            [0, 0, 0, 0],
            "the duplicated padding is zeroed"
        )
    }
    ...
    func testManySameFillSiblingRowsAreKept() {
        func row() -> FigmaNode {
            FigmaNode(
                ...
                w: 300,
                h: 36,
                fill: RGBA(r: 1, g: 1, b: 1, a: 1),
                fillToken: "primaryBackground",
                layout: columnLayout(pad: 8),
                children: [
                    FigmaNode(
                        tag: "text",
                        x: 0,
        ...
        for (index, row) in result.children.enumerated() {
            XCTAssertEqual(
                row.fillToken,
                "primaryBackground",
                "row \(index) keeps its fill"
            )
            XCTAssertEqual(
                row.layout?.pad,
                [8, 8, 8, 8],
                "row \(index) keeps its insets"
            )
        }
    }
    ...
    func testNestedContainerWithADifferentFillIsUntouched() {
        let card = FigmaNode(
            ...
            w: 200,
            h: 80,
            fill: RGBA(r: 0.9, g: 0.9, b: 0.9, a: 1),
            fillToken: "secondaryBackground",
            layout: columnLayout(pad: 12),
            children: []
        )
        ...
        let result = PinDisplayListCapture.stripDuplicateNestedBackground(screen)
        XCTAssertEqual(
            result.children[0].fillToken,
            "secondaryBackground",
            "a distinct nested fill is kept"
        )
    }
}
