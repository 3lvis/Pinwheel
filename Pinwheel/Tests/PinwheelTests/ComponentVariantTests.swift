...
final class ComponentVariantTests: XCTestCase {
    ...
    func testSameStructureRowsWithDifferentImagesGroup() {
        ...
        XCTAssertEqual(
            Set(keys.compactMap { $0 }).count,
            1,
            "same-structure rows group as ONE component even though each has a different image"
        )
    }
    ...
    func testSameStructureRowsWithDifferentFillsGroup() {
        ...
        XCTAssertEqual(
            Set(keys.compactMap { $0 }).count,
            1,
            "same-structure rows group even when a pill's fill colour differs"
        )
    }
    ...
    func testSameStructureRowsWithDifferentChipWidthsGroup() {
        ...
        let parent = FigmaNode(
            ...
            w: 300,
            h: 240,
            children: [row(40), row(64), row(96)]
        )
        let result = PinDisplayListCapture.componentizeRepeatedChildren(parent)
        XCTAssertEqual(
            Set(result.children.compactMap { $0.component }).count,
            1,
    ...
    func testSupersetRowIsMasterAndSubsetRowsHideTheExtraChip() {
        ...
        XCTAssertEqual(
            Set(keys.compactMap { $0 }).count,
            1,
            "one-chip and two-chip rows share ONE component"
        )

        let oneChipPillRow = result.children[0].children[0]
        XCTAssertEqual(
            oneChipPillRow.children.count,
            2,
            "the one-chip row gains the second-chip placeholder"
        )
        XCTAssertEqual(
            oneChipPillRow.children.last?.hidden,
            true,
            "the inserted second chip is hidden"
        )

        let twoChipPillRow = result.children[2].children[0]
        XCTAssertEqual(twoChipPillRow.children.count, 2)
        XCTAssertNotEqual(
            twoChipPillRow.children.last?.hidden,
            true,
            "the superset row keeps both chips visible"
        )
    }
    ...
    func testNoSaleRowJoinsTheSaleComponentWithHiddenOptionalLayers() {
        ...
        XCTAssertEqual(
            Set(keys.compactMap { $0 }).count,
            1,
            "all four rows share ONE component — the no-sale row is a variant, not its own frame"
        )
        ...
        XCTAssertEqual(
            titleRow.children.count,
            2,
            "the title row gains the SALE pill placeholder"
        )
        XCTAssertEqual(
            titleRow.children.last?.hidden,
            true,
            "the inserted SALE pill is hidden"
        )
        XCTAssertEqual(
            priceRow.children.count,
            2,
            "the price row gains the was-price placeholder"
        )
        XCTAssertEqual(
            priceRow.children.last?.hidden,
            true,
            "the inserted was-price is hidden"
        )
    }
}
