...
- **Catalog groups by concept, worlds are tags.** Sections are concept buckets (`Tokens`/`Components`/`Screens`), not `SwiftUI`-vs-`UIKit` splits; each component's SwiftUI and UIKit takes share a section and are told apart by a `PinTag` (`.swiftUI`/`.uiKit`) chip. Tags also drive a horizontal filter-pill bar under the section picker (shown only when a section has >1 distinct tag; resets on section change). The row is `.contentShape(Rectangle())` so the whole width taps, and carries `.accessibilityIdentifier(item.id)` so UI tests navigate by stable id, not title (titles now repeat within a section).
- **Typed catalog identifiers are demo-side, not library.** The demo owns `Component`/`CatalogSection` enums (rawValue = display title) plus thin `PinwheelItem`/`PinwheelSection` convenience inits over them, so call sites read `PinwheelItem(.font).tags(.uiKit)` with no title/id literals. Leading-dot works because those inits take a *concrete* enum; the library can't provide it generically, so it keeps its `String` init and the sugar lives in `CatalogIdentifiers.swift` (a bridge file, so it may `import UIKit`).
