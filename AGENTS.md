...
CI gates every pull request: oida on Linux, and build plus `PinwheelTests` and `DemoTests` on `xcode-27`.
Run both tiers locally before pushing anyway — a local green predicts CI, and the trailer is what the
hook checks.
