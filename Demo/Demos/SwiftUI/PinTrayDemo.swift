...
struct PinTrayDemo: View {
    ...
    private var boost: PinTray {
        PinTray("Boost Post") {
            ...
            PinTrayLink("By clicking the Boost Post button below, you agree to our", phrase: "Terms and Conditions") {}
                .font(.footnote)
        }
        .titleAccessory {
            SwiftUI.Button {
    ...
    private var howItWorks: PinTray {
        PinTray("How it works") {
            ...
            PinTrayText("Select your boost tier and watch your post go viral. Boosted posts are labelled as boosted.")
                .centred()
                .task {
        ...
        PinTray("Pay with") {
            PinTraySection {
                ForEach(Array(methods.enumerated()), id: \.offset) { index, way in
                    PinTrayChoice(
                        way.name,
                        systemImage: way.icon,
                        isChosen: index == method
                    ) {
                        method = index
                        path.removeLast()
                    }
