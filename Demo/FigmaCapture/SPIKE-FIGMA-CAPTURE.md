...
- **Scroll-and-stitch is the rasterized fallback** for lazy content that can't lay out eagerly — chiefly
  the real `UIPinTableView` demo (`uikit-tableview`, a genuine recycling `UITableView`). When a captured
...
- **UIKit table (`UIPinTableView`)** — scroll-stitch is scroll-view-agnostic, so it should capture it, but
...
- **Capture real `UITableView` / `UICollectionView` / `List` — not just eager SwiftUI stacks.** *DONE for `UITableView` (`PinUIKitListCapture`); `UICollectionView` and real `List` still open.* The `UIPinTableView` demo now captures editable text + icons + switches + chevrons + separators, matching the SwiftUI `PinList` capture, with zero cell cooperation. See the "UIKit `UITableView`/`UICollectionView` captures by force-realizing every cell" decisions in `AGENTS.md` for the mechanism and the four gotchas (table-drawn chrome outside `contentView`, tight text rect, safe-area lift, one-booted-sim). Still open: `UICollectionView` frame-geometry transposition (arbitrary/compositional layouts — the mechanism is layout-agnostic via `layoutAttributesForElements`, untested), and reaching SwiftUI-in-cell content (`UIHostingConfiguration` / real `List`) via the `_viewDebugData` probe. *Original feasibility notes below (still accurate for the open parts).* Today capturable surfaces must be eager (`ScrollView { VStack }`) because a recycled, UIKit-backed collection realizes only its visible viewport, so an off-screen host sees nothing. The insight that unlocks it: in a demo/catalog context the data is small (≈5–30 rows), so we don't need recycling at all — we can *force every cell to realize at once* and capture the real UIKit layer. This would let a consumer's existing table/collection/`List` screen capture as-is, which is the whole scaling thesis.
